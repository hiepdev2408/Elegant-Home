                    ...
                    <div class="col">
                        <div class="product__details--info">
                            <form action="#">
                                ...
                                @foreach ($product->productAttributes->groupBy('group_id') as $groupId => $productAttributes)
                                @php
                                    // Lấy giá từ nhóm đầu tiên (nếu có)
                                    $firstGroup = $productAttributes->first();
                                @endphp
                                
                                    <div class="product__details--info__price mb-10">
                                        @if ($firstGroup && $firstGroup->group)
                                        <span class="current__price">{{ $firstGroup->group->price_sale }}VNĐ</span>
                                        <span class="old__price">{{ $firstGroup->group->price }}VNĐ</span>
                                        @endif
                                    </div>
                                ...
                                <div class="product__variant">
                                    <div class="product__variant--list mb-20">
                                        <fieldset class="variant__input--fieldset">
                                            @foreach ($product->productAttributes->groupBy('attribute_id') as $attributeProducts)
                                            @php
                                                // Lấy thuộc tính đầu tiên trong nhóm
                                                $attributes = $attributeProducts->first();
                                            @endphp
                                        
                                            <legend class="product__variant--title mb-8">{{ $attributes->attribute->name }}</legend>
                                        
                                            <ul class="variant__size d-flex">
                                                @foreach ($attributeProducts as $attribute)
                                                    <li class="variant__size--list">
                                                        <input id="color-{{ $attribute->id }}" name="value" type="radio" value="{{ $attribute->value }}" checked>
                                                        @if ($attributes->attribute->name === 'Màu sắc')
                                                            <label class="variant__size--value" for="color-{{ $attribute->id }}" style="background: {{ $attribute->value }};"></label>
                                                        @else
                                                            <label class="variant__size--value">{{ $attribute->value }}</label>
                                                        @endif
                                                    </li>
                                                @endforeach
                                            </ul>
                                        ...
                                        <div class="product__variant--list quantity d-flex align-items-center mb-20">
                                            <div class="quantity__box d-flex align-items-center">
                                                ...
                                                <label class="quantity__label">
                                                    <input type="number" id="quantity-input" class="quantity__number quickview__value--number" value="1" min="1" aria-label="Quantity" />
                                                </label>
                                    ...
                                    <div class="product__variant--list mb-15">
                                        <div class="product__details--info__meta">
                                            ...
                                            <p class="product__details--info__meta--list"><strong>SKU:</strong>
                                                <span>{{$firstGroup->group->SKU}}</span> </p>
                                            <p class="product__details--info__meta--list"><strong>Số lượng:</strong>
                                                <span>{{$firstGroup->group->stock}}</span> </p>
                                            @endforeach
                        ...
                        <div class="swiper-slide">
                            ...
                            <div class="product__items ">
                                ...
                                <div class="product__items--thumbnail">
                                    <a class="product__items--link" href="{{ route('productDetail', ['category_id' => $productSimilar->categories->first()->id, 'id' => $productSimilar->id, 'slug' => $productSimilar->slug]) }}">
                                        <img class="product__items--img product__primary--img" src="{{ Storage::url($productSimilar->img_thumbnail) }}" alt="product-img">
