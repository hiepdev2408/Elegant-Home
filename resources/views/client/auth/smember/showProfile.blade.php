        ...
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="card shadow-sm border-0 rounded-lg p-4">
                    ...
                    <form action="{{ route('profile.info.update', ['id' => $user->id]) }}" method="POST"
                        ...
                        <div class="mb-3">
                            ...
                            @if (Auth::user()->avatar)
                                <img src="{{ Storage::url(Auth::user()->avatar) }}" width="100px" alt=""
                        ...
                        <div class="mb-3">
                            ...
                            <select name="province_id" id="province" class="form-select">
                                ...
                                @foreach ($provinces as $code => $name)
                                    <option @selected($user->province && $user->province->code == $code) value="{{ $code }}">{{ $name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-3">
                            ...
                            <select name="district_id" id="district" class="form-select"
                                @if (!$user->district) disabled @endif>
                                <option value="">Chọn Quận/Huyện</option>
                                @if ($districts)
                                    @foreach ($districts as $id => $name)
                                        <option @selected($user->district && $user->district->id == $id) value="{{ $id }}">{{ $name }}
                                        </option>
                                    @endforeach
                                @endif
                            </select>
                ...
                if (provinceCode) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            $('#ward').empty().append(
                                '<option value="">Chọn Xã/Phường</option>').prop('disabled',
                                true);
                        }
                    });
                } else {
                    $('#district').empty().append('<option value="">Chọn Quận/Huyện</option>').prop(
                        'disabled', true);
